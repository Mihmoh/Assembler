...
putPos MACRO whatPos
	...
	mov ah, 42h                 ; bx - ид файла, cx:dx на сколько переместить, al - точка отсчёта
	xor al, al 			        ; 
...
main:
	...
	mov cl, ds:[80h]		; в 80h находится длина командной строки
	mov cmdSize, cl 		
	
	mov si, 81h    			; переписать командную строку         
	lea di, cmdBuffer       ; с 81h начинается сама командная строка 
	cld  ; clear DF
	rep movsb               ; Записать по адресу ES:(E)DI блок из (E)CX байт, считываемый по адресу DS:(E)SI
                            
	mov ds, ax              
                            
	println startMessage       ;вывести startMessage
                           
	call parseCMD           ;go to 172
	cmp ax, 0               
	jne endMain				
                            
	call openFiles         ;go to 270 
	cmp ax, 0               
	jne endMain				
                            
	call processingFile    ;go to 336  
	cmp ax, 0               
	jne endMain				
...
getCmdWord PROC         ;получить одно слово из командной строки     
	push ax                     
	push cx                     
	push di                     
...
stopSymbol:                
	...
	inc si                      ; Увеличиваем si для перехода на следующий символ 
...
checkTxt_OK:                                            
	...
	mov al, 02h			        ; режим
	lea dx, sourcePath          ; ds:dx адрес asciz-строки с полным именем файла, cl - маска атрибутов файла
	mov cx, 00h			        
	int 21h                     
...
processingFile PROC                    
    ...
    call readFromFile                   ;go to 546
	                                    
	cmp ax, 0                           
	je finishProcessing                 ; когда в ax ноль, то есть ничего уже не прочиталось, то всё
...
p_writeDelims PROC                      ;return to 363  
	push dx                             
	xor bx, bx                          
	xor dx, dx                          
...
isDelim:                                
	movsb                               ;скопировать символ из источника в приёмник
...
checkEndBuff PROC               ;return to 373
	cmp cx, 0                  
...
writeToFile PROC                      
	...
	mov ah, 40h                       ; bx - ид файла, cx - число байт для записи, ds:dx - буфер
	mov bx, sourceID                  
	lea dx, buffer                    
	int 21h   
...
readFromFile PROC                   ;go to 351
	...
	mov ah, 3Fh                     ; bx - ид файла, cx - число байт для чтения ds:dx - буфер
	mov bx, sourceID                
	mov cx, bufferSize             
	lea dx, buffer                  
	int 21h                         

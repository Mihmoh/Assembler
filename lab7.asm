...
main:
    call changeMemoryBlock      ;go to 101
    mov ax, @data                 
	call copyFilePath       ;go to 113
    call openFile       ;go to 147
    ...
    runProgramsLoop: 
        ...
        checkEnter:   
            ...
            inc al
            mov programPathLength, al       ;loop to know size of string before enter
    jmp runProgramsLoop
    
        readProgramPath:
        
        mov si, offset stringBeginIndex
        call setPointer     ;go to 204
        mov cl, programPathLength
        mov dx, offset programPath 
        call readFile            
        call runProgram     ;go to 245
        mov ah, 2    
        mov al, 1
        call movePointer
        mov si, offset stringBeginIndex
...
runProgram proc
    ...
    clearProgramPath:  
        ...
        xor cx, cx
        mov cl, programPathLength
        rep stosb       ;«аполнить блок из (E)CX байт по адресу ES:(E)DI содержимым AL
        ret
runProgram endp
